...
public class UserFrostController {
    ...
    public Result excelExport() {
        try {
            ...
            userFrostService.readyExcelExport(username);
        ...
        @RequestMapping("excelExportProduct")
        public Result excelExportProduct() {
            try {
                String username = SecurityContextHolder.getContext().getAuthentication().getName();
                userFrostService.readyExcelExportProduct(username);
                return new Result(true,"导出成功");
            } catch (Exception e) {
                e.printStackTrace();
                return new Result(false,"导出失败");
        ...
        public Result excelExportOrder() {
            try {
                String username = SecurityContextHolder.getContext().getAuthentication().getName();
                userFrostService.readyExcelExportOrder(username);
                return new Result(true,"导出成功");
            } catch (Exception e) {
                e.printStackTrace();
                return new Result(false,"导出失败");
